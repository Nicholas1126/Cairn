...
为让 B 的图查询真正可用,worker 环境需有 `graphify` 与 `codegraph` 两个 CLI。两端分别保障:

### Dep-1 — Docker 镜像装入工具(`container/Dockerfile`)
在现有 npm/pip 安装区追加:
- graphify:`pip3 install graphifyy --break-system-packages`(PyPI 包名 `graphifyy`,CLI 为 `graphify`;需 Python 3.10+,镜像已满足)
- codegraph:`sudo npm install -g @colbymchenry/codegraph`(CLI 为 `codegraph`;与现有 agent 同样 npm 全局装)
- (可选)`codegraph install`(接 MCP);本轮 worker 用 `codegraph` CLI 查询即可,MCP 接线非必需,先不加。
这样 docker 后端的 worker 容器内 `graphify`/`codegraph` 即就绪。
...
- **前端 `/engines` 页轻量重排**:分两组——
  ...
  - **Tools(依赖)**:graphify、codegraph(来自 `/tools`)。未就绪(`launchable=false`)显著**告警**,文案提示"请安装:`uv tool install graphifyy` / `npm i -g @colbymchenry/codegraph`"。
- 仅"分类 + 告警"的轻改;不改 Agents 既有逻辑。
- 注:依赖工具**要求用户自行安装**(local 模式);平台只探测 + 告警,不代装。

### 兜底
B 的 `{project_knowledge}` 指令在工具缺失时仍引导 agent **退化为直接读** `./project/codegraph-out`、`./project/graphify-out` 下文件(知识是增强,非任务必需)。

## 假设与约束(写入文档)

- **工具就绪**:docker 由 Dep-1 在镜像内装好;local 由用户自行安装、Dep-2 探测并告警。两者缺失时指令引导 agent 退化为读文件。
...
- Dep-2 `probe_tool`/`GET /tools`:mock PATH/which/--version,graphify/codegraph 就绪与缺失两种;`/tools` 返回两项;不泄漏无关信息。
- Dep-1:Dockerfile 改动无单测(构建期),由文档记录;CI/构建验证。
- 前端 Dep-2:手测(/engines 页 Agents/Tools 两组渲染、缺失告警与安装提示)。
...
- B 主体:`server/db.py`(列)、`server/models.py`(`project_root`)、`server/routers/projects.py` + `services.py`(存读 + 校验)、`server/static/index.html`(New Project 字段)、`dispatcher/runtime/base.py`(`ensure_running` 签名)、`dispatcher/runtime/containers.py`(只读 bind-mount)、`dispatcher/runtime/local/runtime.py`(软链接)、`dispatcher/tasks/{bootstrap,reason,explore}.py`(传 `project_knowledge` + `ensure_running` 传 `project_root`)、`dispatcher/tasks/common.py`(`prepare_project_knowledge`)、`prompting.py`(`format_project_knowledge`)、`prompts/default/*.md`(`{project_knowledge}` 占位)、测试 fake runtime 的 `ensure_running` 签名。
- Dep-1:`container/Dockerfile`(追加 `pip3 install graphifyy --break-system-packages` + `npm install -g @colbymchenry/codegraph`)。
- Dep-2:`dispatcher/runtime/local/resolve.py`(`TOOLS` + `probe_tool`)、`server/models.py`(`ToolInfo`)、`server/routers/engines.py`(或新 `tools.py`,`GET /tools`)、`server/static/index.html`(/engines 页 Agents/Tools 分组 + 缺失告警)、`resolve`/`/tools` 测试。
- 文档:server-protocol / dispatcher-design / README 增补(含依赖工具安装说明)。
