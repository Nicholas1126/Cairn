...
任务开始前，Dispatcher 调用 `Runtime.install_skills` 将所有已启用技能安装到 Agent 工作区的 `.claude/skills/` 目录：

- **Docker 后端**：将技能目录打包后通过 Docker tar `put_archive` API 上传至容器内 `/home/kali/workspace/.claude/skills/`。
- **宿主机后端**：通过 `shutil.copytree` 将技能目录复制到宿主机工作区 `~/.cairn/workspaces/<project_id>/.claude/skills/`。

技能同时通过 `{skills}` 占位符（`format_skills`）渲染到 `bootstrap`、`reason`、`explore` 等阶段的 prompt 中，告知 Agent 当前可用技能的名称、描述和文件路径。claude 可通过 `.claude/skills/` 原生发现技能文件；codex、opencode、pi 则依赖 prompt 指令与文件内容读取技能。两类后端（Docker 和宿主机）均支持 Skills 注入。

### 项目知识注入（Project Knowledge Injection）

每个项目可选配置 `project_root`（宿主机绝对路径，对应字段 `ProjectMeta.project_root`），指向一个统一布局的项目根目录 A。该目录下的固定子目录约定如下：

| 子目录 | 内容 |
| --- | --- |
| `src-repo/` | 源码 |
| `docs-out/` | 产品文档 / 资料 |
| `graphify-out/` | 领域知识图谱（graphify 输出） |
| `scan-out/` | 静态扫描结果 |
| `codegraph-out/` | 代码图（codegraph 输出） |

#### 只读挂载到 `./project`

A 在容器创建时以只读方式共享进工作区，两类后端均对外暴露相同的相对路径 `./project`：

- **Docker 后端**：在 `ensure_running(project_id, project_root)` 创建容器时，将 A bind-mount（`mode=ro`）到容器内 `/home/kali/workspace/project`。
- **宿主机后端**：在 `ensure_running(project_id, project_root)` 初始化工作区时，在工作区目录下创建软链接 `project -> A`（仅在链接不存在时创建；若已存在则跳过）。

挂载在容器/工作区**创建时**完成。事后修改 `project_root`（通过前端编辑项目）不会热更新现有容器；若需生效，必须先停止并重建容器。

#### `{project_knowledge}` 占位符

任务派发前，Dispatcher 调用 `prepare_project_knowledge(project_root)`（`cairn/src/cairn/dispatcher/tasks/common.py`）探测 A 下实际存在哪些子目录，再由 `format_project_knowledge`（`cairn/src/cairn/dispatcher/prompting.py`）渲染 `{project_knowledge}` 占位符，并注入到 `bootstrap`、`reason`、`explore` 等阶段的 prompt 中。

渲染规则：

- 只列出实际**存在**的子目录，不列出缺失的目录。
- 每类子目录附带一行用法指令（例如：如何用 `graphify` 查询图谱、如何用 `codegraph` 查询代码图、如何直接读文件）。
- 统一说明"优先复用，勿重复前期分析"，对应工具缺失时退化为直接读文件。
- `project_root` 为 `null` 或 A 下无任何已知子目录时，占位符渲染为空字符串，不在 prompt 中产生任何内容。

渲染后的文本示例（A 下有 `src-repo`、`graphify-out`、`codegraph-out` 时）：
...
- source code: read / grep `./project/src-repo`
- domain knowledge graph: run `graphify query "<question>"` over `./project/graphify-out`
- code graph: query with the `codegraph` CLI (query / explore / node / callers / impact) over `./project/codegraph-out`
```

#### 依赖工具

| 工具 | 用途 | Docker 镜像安装方式 | 宿主机后端 |
| --- | --- | --- | --- |
| `graphify` | 领域知识图谱查询 | `pip3 install graphifyy --break-system-packages` | 用户自行安装 |
| `codegraph` | 代码图查询 | `npm install -g @colbymchenry/codegraph` | 用户自行安装 |

Docker 后端：两个工具已内置于镜像，Agent 开箱即用。

宿主机后端：工具由用户自行在宿主机上安装。平台通过 `GET /tools` 探测就绪状态，并在 /engines 页面的 **Tools** 分组中展示；若工具未安装（`launchable: false`），前端会显示安装警告。
