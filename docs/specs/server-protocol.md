...
execution_record_enabled   # 是否启用执行记录，默认 true；为 false 时 POST /executions 返回 204
execution_file_logging     # 是否将完整执行日志写入磁盘，默认 true；为 false 时仅保存行内截断输出
```
...
id
project_id
phase           # "bootstrap" | "reason" | "explore" | "conclude"
intent_id       # 关联的 intent id，可选
worker_name     # 执行该任务的 Worker 名称
model           # 使用的模型名称，可选
outcome         # 执行结果标识，由上报方定义
started_at
ended_at
duration_ms
has_log         # 是否存在对应的 .log 文件
command_preview # 已脱敏的执行命令预览（最多 8192 字符，供 Runtime 列表展示）
```

### ExecutionDetail

```
（包含 ExecutionSummary 的所有字段，另外附加）
command         # 实际执行的命令数组，已脱密
prompt          # 渲染后的 prompt 文本，已脱密
response_text   # 模型响应文本（行内截断），已脱密，可选
stdout          # 标准输出（行内截断），已脱密
stderr          # 标准错误（行内截断），已脱密
exit_code       # 进程退出码，可选
stdout_bytes    # stdout 实际字节数
stderr_bytes    # stderr 实际字节数
truncated       # 行内输出是否被截断
log_path        # 磁盘 .log 文件绝对路径，无日志时为 null
produced_fact_id      # conclude 阶段产出的 fact id，可选
produced_intent_ids   # reason 阶段产出的 intent id 列表
```
...
执行记录由 Dispatcher 在每次任务运行结束后上报，用于审计和问题排查。服务端在存储前会对命令、prompt 和输出中的敏感字段（apiKey / token / Bearer 凭证）做脱密处理，并将行内输出截断至最多 64KB（头尾各保留一半）。若磁盘日志已启用，服务端还会将完整输出（最多 10MB）写入 `<data_dir>/executions/<project_id>/` 目录下的 `.log` 文件，并在记录中保存其绝对路径 `log_path`。执行记录随项目级联删除：删除项目时，对应的数据库行和磁盘 `executions/<project_id>/` 目录会一并清除。

#### POST /projects/{project_id}/executions

上报一条执行记录。当 `execution_record_enabled=false` 时返回 `204 No Content`，不存储任何数据；否则返回 `201 Created` 并附带已存储的 ExecutionDetail。
...
```json
{
  "phase": "explore",
  "intent_id": "i002",
  "worker_name": "claude-sonnet-doer",
  "model": "claude-sonnet-4-6",
  "command": ["claude", "--session-id", "...", "-p", "--", "..."],
  "prompt": "...",
  "response_text": "...",
  "stdout": "...",
  "stderr": "...",
  ...
  "ended_at": "2026-03-21T10:02:00Z",
  "duration_ms": 60000,
  "produced_fact_id": "f001",
  "produced_intent_ids": []
}
```
...
- `phase`：`bootstrap`、`reason`、`explore`、`conclude` 之一
- `intent_id`：关联的 intent id，可选
- `model`：使用的模型名称，可选
- `response_text`：模型响应文本，可选
- `exit_code`：进程退出码，可选
- `produced_fact_id`：conclude 阶段产出的 fact id，可选
- `produced_intent_ids`：reason 阶段产出的 intent id 列表，可为空数组

---

#### GET /projects/{project_id}/executions

返回该项目的执行记录列表（ExecutionSummary 格式，不含完整输出），按 `started_at` 升序排列。
...
#### GET /projects/{project_id}/executions/{exec_id}

返回单条执行记录的完整详情（ExecutionDetail 格式），含行内命令、prompt、截断后的 stdout/stderr、字节数、截断标志和日志路径。
...
```json
{
  ...
  "ended_at": "2026-03-21T10:01:00Z",
  "duration_ms": 60000,
  "command": ["claude", "--session-id", "abc123", "-p", "--", "..."],
  "prompt": "...",
  "response_text": "...",
  "stdout": "...",
  "stderr": "...",
  ...
  "stderr_bytes": 0,
  "truncated": false,
  "log_path": "/data/executions/proj_001/exec_001.log",
  "has_log": true,
  "produced_fact_id": "f001",
  "produced_intent_ids": []
}
...
#### GET /projects/{project_id}/executions/{exec_id}/log

下载单条执行记录对应的 `.log` 文件原始内容。若该记录不存在日志文件，返回 `404`。

---

#### GET /projects/{project_id}/executions/logs.zip

下载该项目所有执行记录的 `.log` 文件打包成的 ZIP 压缩包。若该项目没有任何日志文件，返回 `404`。
